...
export const studentuploadStudentQuiz = async (
    quizid: string,
    userid: string,
    selectedFile: File
) => {
...
export const saveStudentQuiz = async (
    ...
    try {
        const response = await axiosInstance.put("/api/studentquiz/edit", {
            studentQuizId,
            userId,
            newText,
            comment,
...
export const approveQuizAnswer = async (
    studentQuizId: string,
    userId: string,
    studentId: string,
    quizId: string,
...
export const disapproveQuizAnswer = async (
    studentQuizId: string,
    userId: string,
    studentId: string,
    quizId: string,
...
export const deleteStudentQuiz = async (studentId: string, quizId: string) => {
    try {
        const response = await axiosInstance.delete("/api/studentquiz/delete", {
        ...
        if (axios.isAxiosError(error)) {
            const errorMessage =
                error.response?.data || "Error deleting the score";
            toast.error(errorMessage);
        } else {
            toast.error("An unexpected error occurred");
        }
...
export const recordActivityLog = async (
    ...
    try {
        ...
        if (axios.isAxiosError(error)) {
            const errorMessage =
                error.response?.data || "Error logging activity";
            toast.error(errorMessage);
        } else {
...
export const getAllActivityLogs = async (studentQuizId: string) => {
    try {
        const response = await axiosInstance.get("/api/studentquiz/logs/all", {
            params: {
                studentQuizId,
            },
        });
        ...
        if (axios.isAxiosError(error)) {
            const errorMessage =
                error.response?.data || "Error retrieving activity logs";
            toast.error(errorMessage);
        } else {
            toast.error(
...
export const addFeedbackToEditedAnswerPerItem = async (
    studentQuizId: string,
    itemId: number,
    feedback: string
) => {
    try {
        ...
        if (axios.isAxiosError(error)) {
            const errorMessage =
                error.response?.data || "Error adding feedback";
            toast.error(errorMessage);
        } else {
