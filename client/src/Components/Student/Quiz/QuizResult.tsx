...
const StudentQuizResults = () => {
    ...
    const [formattedDueDate, setFormattedDueDate] = useState<string | null>(
        null
    );
    ...
    const [
        ...
        setShowFeedbackPerItemModalDisplay,
    ] = useState(false);
        ...
        for (let i = 1; i <= maxItemNumber; i++) {
            ...
            const correctAnswer =
                dueDateTime && dueDateTime < currentDate
                    ? correctAnswers.find((ans) => ans.itemnumber === i)
                          ?.answer || ""
                    : "";
