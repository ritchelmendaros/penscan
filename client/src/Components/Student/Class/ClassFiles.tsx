    ...
    useEffect(() => {
        const fetchQuizData = async () => {
            if (clickedClass?.classid) {
                try {
                    const quiz: Quiz[] = await getQuizzesByClassId(
                        clickedClass.classid
                    );

                    const quizzesWithProperMapping: Quizzes[] = quiz.map(
                        (q) => ({
                            quizId: q.quizid,
                            quizName: q.quizname,
                    ...
                    if (user?.userid) {
                        ...
                        const filteredResults = results.filter(
                            (result): result is StudentImageResult =>
                                result !== null
                        );
